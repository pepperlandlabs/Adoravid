...
NSString * vidtitle = @"via @Adoravid";
...
{
    if ([touch.view isDescendantOfView:self.videoPlayerView.playerControlBar] || [touch.view isDescendantOfView:self.videoPlayerView.playerTopBar]) {
        return NO;
    }
    return YES;
}
...
{
    showShareOptions = NO;
    [ShareThis showShareOptionsToShareUrl:[_currentVideoInfo objectForKey:@"shareURL"] title:vidtitle image:nil onViewController:[[[UIApplication sharedApplication] keyWindow] rootViewController] forTypeOfContent:STContentTypeVideo];
...
- (void)showControls
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        self.videoPlayerView.playerTopBar.alpha = 1.0;
...
- (void)hideControlsAnimated:(BOOL)animated
{
    ...
    if (animated) {
        [UIView animateWithDuration:0.4 animations:^{
            ...
            self.videoPlayerView.playerTopBar.alpha = 0;
...
{
    if ([self isPlaying]) {
        [_videoPlayerView.playPauseButton setImage:[UIImage imageNamed:@"pause-button3"] forState:UIControlStateNormal];
    } else {
        [_videoPlayerView.playPauseButton setImage:[UIImage imageNamed:@"play-button3"] forState:UIControlStateNormal];
    }
}
...
- (void)syncFullScreenButton:(UIInterfaceOrientation)toInterfaceOrientation
{
    if (_fullScreenModeToggled) {
        [_videoPlayerView.fullScreenButton setTitle:@"Done" forState:UIControlStateNormal];
    } else {
        [_videoPlayerView.fullScreenButton setTitle:@"Done" forState:UIControlStateNormal];
    }
}
